(() => {
  ...
  var add = (dom, ...children) => {
    for (let c of children.flat(Infinity)) {
      ...
      if (protoOfC === stateProto) {
        ...
        child = bind(() => {
          let { val: val2 } = c;
          return val2 == _undefined ? _undefined : (dom2.textContent = c.val, dom2);
        });
      } else
        child = protoOfC === funcProto ? bind(c) : c;
      if (child != _undefined)
        dom.append(child);
    }
    return dom;
  ...
  var state2 = (initVal) => new Proxy(van_default.state(Object.freeze(checkStateValValid(initVal))), {
    set: (s, prop, val2) => (prop === "val" && Object.freeze(checkStateValValid(val2)), Reflect.set(s, prop, val2))
  });
